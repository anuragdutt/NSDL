...
path.static <- "/home/nsdl/RESULTS/NESTLE_HOLDINGS/"
isin <- "INE239A01016"
nifty.var   <- NULL
niftyjr.var <- NULL
both.var    <- NULL
...
for(dt in date.seq) {
    print(dt)
    file.path <-
        paste0(path.static, "nestle_client_holdings_",
               dt, ".Rdata")
    load(file.path)

    age.diff <- result[!duplicated(result$client_id),
                       c("client_id", "dp_id", 
                         "difference", "age_account")]
                                        # converting age in days to
                                        # years and categorising on 5
                                        # year intervals
    ...
    age.diff$status <- NA
    age.diff$status[which(age.diff$difference < 0)] <- "s"
    age.diff$status[which(age.diff$difference > 0)] <- "b"
    age.diff$status[which(age.diff$difference == 0)] <- "nt"

                                        # Summary statistics based on
                                        # age categories
    df.a <- as.data.frame.matrix(table(age.diff$status, age.diff$age_bracket))
    df.p <- as.data.frame.matrix(prop.table(table(age.diff$status, age.diff$age_bracket), 2)*100)
    colnames(df.a) <- c("0-5", "5-10", "10-15", ">15")
    colnames(df.p) <- c("0-5", "5-10", "10-15", ">15")
    fn.a <- paste0(path.static, "table_a_age_trade_", dt, ".Rdata")
    fn.p <- paste0(path.static, "table_p_age_trade_", dt, ".Rdata")
    save(df.a, file = fn.a) 
    save(df.p, file = fn.p)
    age.dat  <- age.diff[, c("client_id", "age_bracket")]
    isin.dat <- result[which(result$isin == isin), c("client_id",
                                                     "qty")]
    age.merge <- merge(age.dat, isin.dat, by = "client_id", all.x =
                                                                TRUE)
    ...
    nifty.diff$status <- NA
    nifty.diff$status[which(nifty.diff$difference < 0)] <- "s"
    nifty.diff$status[which(nifty.diff$difference > 0)] <- "b"
    nifty.diff$status[which(nifty.diff$difference == 0)] <- "nt"
    
    df.a <- as.data.frame.matrix(table(nifty.diff$status, nifty.diff$nifty_bracket))
    df.p <- as.data.frame.matrix(prop.table(table(nifty.diff$status,
                                                  nifty.diff$nifty_bracket), 2)*100)
    colnames(df.a) <- c("0-25", "25-50", "50-75", ">75")
    colnames(df.p) <- c("0-25", "25-50", "50-75", ">75")
    fn.a <- paste0(path.static, "table_a_nifty_trade_", dt, ".Rdata")
    fn.p <- paste0(path.static, "table_p_nifty_trade_", dt, ".Rdata")
    save(df.a, file = fn.a) 
    save(df.p, file = fn.p)
    nifty.dat  <- nifty.diff[, c("client_id", "nifty_bracket")]
    isin.dat <- result[which(result$isin == isin), c("client_id",
                                                     "qty")]
    nifty.merge <- merge(nifty.dat, isin.dat, by = "client_id", all.x =
                                                                TRUE)
    ...
    niftyjr.diff$status <- NA
    niftyjr.diff$status[which(niftyjr.diff$difference < 0)] <- "s"
    niftyjr.diff$status[which(niftyjr.diff$difference > 0)] <- "b"
    niftyjr.diff$status[which(niftyjr.diff$difference == 0)] <- "nt"
    
    df.a <- as.data.frame.matrix(table(niftyjr.diff$status, niftyjr.diff$niftyjr_bracket))
    df.p <- as.data.frame.matrix(prop.table(table(niftyjr.diff$status,
                                                  niftyjr.diff$niftyjr_bracket), 2)*100)
    colnames(df.a) <- c("0-25", "25-50", "50-75", ">75")
    colnames(df.p) <- c("0-25", "25-50", "50-75", ">75")
    fn.a <- paste0(path.static, "table_a_niftyjr_trade_", dt, ".Rdata")
    fn.p <- paste0(path.static, "table_p_niftyjr_trade_", dt, ".Rdata")
    save(df.a, file = fn.a) 
    save(df.p, file = fn.p)
    niftyjr.dat  <- niftyjr.diff[, c("client_id", "niftyjr_bracket")]
    isin.dat <- result[which(result$isin == isin), c("client_id",
                                                     "qty")]
    niftyjr.merge <- merge(niftyjr.dat, isin.dat, by = "client_id", all.x =
                                                                TRUE)
    ...
    niftyjr.var <- rbind(niftyjr.var, niftyjr.v)
                                        ...
                                        # Summary statistics  based on sum of nifty and nifty junior
                                        # categories
    ...
    both.diff$status <- NA
    both.diff$status[which(both.diff$difference < 0)] <- "s"
    both.diff$status[which(both.diff$difference > 0)] <- "b"
    both.diff$status[which(both.diff$difference == 0)] <- "nt"
    
    df.a <- as.data.frame.matrix(table(both.diff$status, both.diff$both_bracket))
    df.p <- as.data.frame.matrix(prop.table(table(both.diff$status,
                                                  both.diff$both_bracket), 2)*100)
    fn.a <- paste0(path.static, "table_a_both_trade_", dt, ".Rdata")
    fn.p <- paste0(path.static, "table_p_both_trade_", dt, ".Rdata")
    colnames(df.a) <- c("0-25", "25-50", "50-75", ">75")
    colnames(df.p) <- c("0-25", "25-50", "50-75", ">75")
    save(df.a, file = fn.a) 
    save(df.p, file = fn.p)
    ...
    isin.dat <- result[which(result$isin == isin), c("client_id",
                                                     "qty")]
    both.merge <- merge(both.dat, isin.dat, by = "client_id", all.x =
                                                                TRUE)
...
dev.off()
                                        ...
                                        # plotting nifty junior distribution
...
dev.off()
                                        ...
                                        # plotting sum of nifty and
                                        # nifty junior distribution
