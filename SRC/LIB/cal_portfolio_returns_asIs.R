...
## load("/home/nsdl/RESULTS/SATYAM/DATA/satyam_dataset_2009-01-06.Rdata")
## holdings.prev <- holdings
## load("/home/nsdl/RESULTS/SATYAM/DATA/satyam_dataset_2009-01-07.Rdata")
## holdings <- holdings
## load("/home/nsdl/CONSTANTS/stock_returns_all.Rdata")
## ret.dat <- stock.ret

##' Calculates individual portfolio values as it is for a day's holdings file
##'
##' Cash in cash out not factored in
...
calPortValParallelised <- function(holdings,
                                            roll.per = 0,
                                            on = "simple",
                                            wt = "qty",
                                            close,
...
calPortValMc <- function(holdings,
                         roll.per = 0,
                         on = "simple",
                         wt = "qty",
                         close,
...
#    library(data.table)    
    ...
    for(i in c(1:10)) {
        if(length(which(ret.dt %in% unique(isin.dat$date))) == 0) {
            ret.dt = ret.dt - 1
        }
    }
    ...
    return.portfolio <- merge(holdings, isin.dat,
                              by = c("date", "isin"),
                              all.x = TRUE)
    return.portfolio <-
        return.portfolio[order(return.portfolio[["client_id"]],
    ...
    return.portfolio$val <-
        return.portfolio$qty*return.portfolio$close

    return.portfolio <- return.portfolio[, val_asis := sum(val),
                                         by = list(client_id)][ ,c("client_id",
                                                                   "val_asis",
                                                                   "date"),
                                                               with = FALSE]

    colnames(return.portfolio) <- tolower(c("CLIENT_ID",
                                            "VAL_ASIS",
                                            "DATE"))
